...
# * objects
# * constructors
# * class methods with def self.methodname
# * factory methods
# * options hashes
#
# # Hints
...
describe Temperature do
  describe 'can be constructed with an options hash' do
    describe 'in degrees fahrenheit' do
      it 'at 50 degrees' do
        expect(described_class.new(f: 50).in_fahrenheit).to eq(50)
      end

      describe 'and correctly convert to celsius' do
        it 'at freezing' do
          expect(described_class.new(f: 32).in_celsius).to eq(0)
        end

        it 'at boiling' do
          expect(described_class.new(f: 212).in_celsius).to eq(100)
        end

        it 'at body temperature' do
          expect(described_class.new(f: 98.6).in_celsius).to eq(37)
        end

        it 'at an arbitrary temperature' do
          expect(described_class.new(f: 68).in_celsius).to eq(20)
        end
      end
    end
    ...
    describe 'in degrees celsius' do
      it 'at 50 degrees' do
        expect(described_class.new(c: 50).in_celsius).to eq(50)
      end

      describe 'and correctly convert to fahrenheit' do
        it 'at freezing' do
          expect(described_class.new(c: 0).in_fahrenheit).to eq(32)
        end

        it 'at boiling' do
          expect(described_class.new(c: 100).in_fahrenheit).to eq(212)
        end

        it 'at body temperature' do
          expect(described_class.new(c: 37).in_fahrenheit).to be_within(0.1).of(98.6)
  ...
  describe 'Temperature subclasses' do
    describe 'Celsius subclass' do
      ...
      it 'is a Temperature subclass' do
        expect(Celsius.new(0)).to be_a(described_class)
      end
    end
    ...
    describe 'Fahrenheit subclass' do
      ...
      it 'is a Temperature subclass' do
        expect(Fahrenheit.new(0)).to be_a(described_class)
      end
    end
  end
end
